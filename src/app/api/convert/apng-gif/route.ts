...
export async function POST(request: NextRequest) {
  try {
    ...
    const name = (file as any).name || 'input.apng';

    const lower = name.toLowerCase();
    if (!(lower.endsWith('.apng') || (file as any).type === 'image/apng')) {
      return NextResponse.json({ error: "Only .apng files are supported" }, { status: 400 });
    }

    const dir = join(tmpdir(), `apng-gif-${Date.now()}`);
    if (!existsSync(dir)) mkdirSync(dir, { recursive: true });

    const input = join(dir, 'input.apng');
    ...
    console.error('apng-gif api error', e);
