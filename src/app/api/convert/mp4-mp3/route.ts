...
export async function GET() {
  return NextResponse.json({
    ...
    message: "MP4 to MP3 converter API - Use POST for conversions"
  });
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const tempDir = join(tmpdir(), 'mp4-mp3-converter');
    ...
    try {
      ...
      return NextResponse.json({ 
        error: "MP4 to MP3 conversion failed. Make sure FFmpeg is installed and the input file is valid." 
      }, { status: 500 });
    }

  } catch (error) {
    console.error('MP4 to MP3 conversion error:', error);
    return NextResponse.json({ error: "MP4 to MP3 conversion failed" }, { status: 500 });
  }
}
