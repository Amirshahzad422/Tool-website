...
export async function POST(request: NextRequest) {
  try {
    ...
    const name = (file as any).name || 'input.avi';
    if (!name.toLowerCase().endsWith('.avi')) {
      return NextResponse.json({ error: "Only .avi files are supported" }, { status: 400 });
    }

    const dir = join(tmpdir(), `avi-gif-${Date.now()}`);
    if (!existsSync(dir)) mkdirSync(dir, { recursive: true });

    const input = join(dir, 'input.avi');
    ...
    console.error('avi-gif api error', e);
