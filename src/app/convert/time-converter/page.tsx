import type { Metadata } from "next";
import TimeConverterClient from "./TimeConverterClient";

export const metadata: Metadata = {
  title: "Time Converter",
  description: "Convert between nanoseconds, milliseconds, seconds, minutes, hours, days, weeks, months, years, decades, centuries, and more time units. Fast, accurate, and easy-to-use time conversion tool.",
  alternates: { canonical: "/convert/time-converter" },
};

export default function TimeConverterPage() {
  return (
    <div className="min-h-screen bg-white">
      <div className="container mx-auto px-4 pt-16 pb-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-6">
            <h1 className="text-4xl font-bold text-gray-900 mb-4">
              Time Converter
            </h1>
            <p className="text-lg text-gray-700 max-w-2xl mx-auto">
              Convert between different time units including nanoseconds, milliseconds, seconds, 
              minutes, hours, days, weeks, months, years, decades, centuries, and more. 
              Includes special time units like leap years, sidereal time, and lunar months.
            </p>
          </div>
          
          <TimeConverterClient />
        </div>
      </div>
    </div>
  );
}
